// <copyright file="ClosedCaptionClient.cs" company="Drastic Actions">
...
{
    private readonly ClosedCaptionController controller;

    /// <summary>
    /// Initializes a new instance of the <see cref="ClosedCaptionClient"/> class.
    /// Initializes an instance of <see cref="ClosedCaptionClient" />.
    /// </summary>
    public ClosedCaptionClient(HttpClient http) =>
        this.controller = new ClosedCaptionController(http);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var trackInfos = playerResponse
            ...
            .Select(t =>
            {
                var url =
                    t.TryGetUrl() ??
                    throw new DrasticYouTubeException("Could not extract track URL.");

                var languageCode =
                    t.TryGetLanguageCode() ??
                    throw new DrasticYouTubeException("Could not extract track language code.");

                var languageName =
                    t.TryGetLanguageName() ??
                    throw new DrasticYouTubeException("Could not extract track language name.");
    ...
    public async ValueTask<ClosedCaptionTrack> GetAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var trackExtractor = await this.controller.GetClosedCaptionTrackAsync(trackInfo.Url, cancellationToken);
            ...
            .Select(c =>
            {
                ...
                var parts = c
                    ...
                    .Select(p =>
                    {
                        ...
                        var partOffset =
                            p.TryGetOffset() ??
                            throw new DrasticYouTubeException("Could not extract caption part offset.");
    ...
    public async ValueTask WriteToAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var track = await this.GetAsync(trackInfo, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await this.WriteToAsync(trackInfo, writer, progress, cancellationToken);
    }
}